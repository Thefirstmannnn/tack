...
const URL_ = 'https://YOUR_DOMAIN/mcp';
...
describe('mcp install links', () => {
  it('installs Claude Code at user scope, so every project on the machine sees it', () => {
    expect(claudeCodeCommand(URL_)).toBe(
      'claude mcp add --scope user --transport http tack https://YOUR_DOMAIN/mcp',
    );
  });
  ...
  it('encodes the Cursor deeplink config as base64 json', () => {
    ...
    expect(
      href.startsWith('cursor://anysphere.cursor-deeplink/mcp/install?name=tack&config='),
    ).toBe(true);

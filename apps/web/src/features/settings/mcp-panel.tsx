...
const MCP_DESCRIPTION =
  'Connect a compatible AI client to Tack. The client signs in with your Tack account and acts as you, within your permissions. No API key needed.';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        <span className="text-2xs text-faint">Add Tack to a client</span>
        ...
        <p className="text-2xs text-faint">
          Ask the client to call get_me once it is connected, to confirm it can reach Tack.
        </p>
      </div>
  ...
  return (
    ...
    >
      ...
      {action.kind === 'url' ? (
        <CopyRow
          value={action.url}
          label={`Copy the Tack server URL for ${client.name}`}
          onError={onError}
        />
      ) : null}

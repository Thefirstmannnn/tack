...
export function claudeCodeCommand(mcpUrl: string): string {
  return `claude mcp add --scope user --transport http tack ${mcpUrl}`;
}

export function cursorInstallHref(mcpUrl: string): string {
  const config = toBase64(JSON.stringify({ url: mcpUrl }));
  return `cursor://anysphere.cursor-deeplink/mcp/install?name=tack&config=${config}`;
}

export function vscodeInstallHref(mcpUrl: string): string {
  const config = JSON.stringify({ name: 'tack', type: 'http', url: mcpUrl });
...
const APPROVE_STEP = 'Sign in to Tack, pick a workspace, and approve.';
  ...
  return [
    {
      ...
      steps: [
        ...
        'Paste the URL, then sign in to Tack, pick a workspace, and approve.',
      ],
    },
    {
      id: 'chatgpt',
      ...
      steps: [
        ...
        'Create the app, then sign in to Tack, pick a workspace, and approve.',
      ],
    },
    {
      id: 'claude-code',
      name: 'Claude Code',
      summary: 'Add Tack once and every session on this machine can reach it.',

...
describe('targetName', () => {
  it('names a database without carrying its credentials', () => {
    const named = targetName('postgres://someone:hunter2@db.example.com:5432/tack');
    expect(named).toBe('db.example.com/tack');
...
const SCRATCH = laneDatabase('tack_test_catchup', currentLane());
...
const SPRINT_SCRATCH = laneDatabase('tack_test_workspace_sprint_catchup', currentLane());
...
const ANALYTICS_SCRATCH = laneDatabase('tack_test_analytics_catchup', currentLane());

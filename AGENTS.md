...
Tack is a free, realtime, keyboard-first task manager. One Next.js app on
...
2. **Shipped server code must not import a Bun built-in.** The deployed runtime
   ...
   `@tack/shared/utils`. Test files and `apps/realtime` are exempt.
...
6. **Strict types.** `any` and non-null assertions are lint errors.
   ...
   external input with a Zod schema from `@tack/shared`.

...
[issues labelled `roadmap`](https://github.com/Thefirstmannnn/tack/labels/roadmap),
...
Tack is built for a keyboard and a large screen, and it shows on a phone. The
...
change that most widens who can use Tack.

### Outbound webhooks

Fire on issue and project events so Tack can drive other systems. The most
requested integration feature, and the MCP server only partly covers it.

### Importers

Tack has validated readers for Plane and Linear exports, plus reusable mapping
and row-building helpers. There is no supported command that writes either
export into an Tack database.
...
requested thing that Tack deliberately does not have yet, because doing it
...
- **A public API** beyond MCP, for people who want to script Tack directly.
...
there is no scaffolding for a future one. This is the whole point of Tack, and
...
**Being a CRM, a helpdesk, a wiki platform or a spreadsheet.** Tack tracks
...
- **Open a feature request.** Describe the workflow that is painful, not the
  widget you want. See
  [CONTRIBUTING.md](https://github.com/Thefirstmannnn/tack/blob/main/CONTRIBUTING.md#proposing-features).
- **Build it.** Comment first so we can agree the shape, then go. Anything
  marked **help wanted** already has agreement.

## Releases

Tack ships continuously from `main`. There are no long lived release branches
...
[`breaking change`](https://github.com/Thefirstmannnn/tack/labels/breaking%20change)

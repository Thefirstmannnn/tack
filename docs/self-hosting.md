...
> **Status: Preview.** This page documents the current mbhatt deployment and
> evaluation paths. Tack does not yet publish a provider-neutral production
...
Everything below has a free tier, so a small team can run Tack for nothing.
...
| Transactional email (optional with password or OAuth) | [Resend](https://resend.com) | None. Tack only supports Resend |
...
run yourself. Tack uses `postgres.js` through Drizzle, and no
...
database created before Tack had a migration ledger is baselined without changing
...
set `TACK_PASSWORD_AUTH=true`, both Google OAuth variables, or both GitHub OAuth
variables. Passkeys cannot bootstrap a new installation, and `TACK_DEV_LOGIN`
...
- **`TACK_DEV_LOGIN`.** It signs anyone in as any user with one click.
...
```bash
curl https://tack.example.com/api/health
```
...
If you want to evaluate Tack inside your own network, run the Next.js
...
indexes are reported and preserved. Tack ships continuously from `main`. We
...
Watch the [releases](https://github.com/Thefirstmannnn/tack/releases) for anything
labelled `breaking change` and follow the upgrade notes in the associated release.

### Backups

Back up Postgres and object storage together. Tack ships a coordinated backup
...
DIRECT_URL="postgres://user:pass@host:5432/tack" bun run backup:create -d ./backups

# Machine-readable output for cron or orchestrators
bun run backup:create --json --destination /var/backups/tack
```
...
| --- | --- | --- | --- |
| `--destination`, `-d` | `TACK_BACKUP_DESTINATION` | `./backups` | Target directory where the backup folder is published |
...
| `--tack-version` | `TACK_VERSION` | `0.1.0` | Tack version string stamped into `manifest.json` |
...
Each backup creates an isolated directory named `tack-backup-<timestamp>-<hash>/`:

```
tack-backup-2026-09-10T19-36-31-839Z-68a1dddb/
...
- **Online object capture:** The database snapshot guarantees consistent relational
  ...
  snapshot began. If external tooling deletes an object from storage while Tack
...
Tack is fine on the smallest tier of everything for a team of twenty. The
...
Read [SECURITY.md](https://github.com/Thefirstmannnn/tack/blob/main/SECURITY.md), which has the full checklist. The short
version:

- Fresh `BETTER_AUTH_SECRET`.
- `TACK_DEV_LOGIN` unset.
